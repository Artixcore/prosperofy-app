...
export function ApiErrorPanel({
  ...
  const code = isApiClientError(error) && !isServerFailure ? resolved.code ?? error.code : undefined;
  ...
  return (
    ...
    >
      ...
      <p className="mt-1 text-sm text-red-700 dark:text-red-200/90">{resolved.message}</p>
      {resolved.hints.map((hint) => (
        <p key={hint} className="mt-1 text-sm text-red-700 dark:text-red-200/90">
          {hint}
        </p>
      ))}
      {code ? (
